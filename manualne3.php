...
preg_match_all('!\d+!', $nazov_suboru, $matches);
...
foreach ($matchess as $matchesss) {
$prepocet = $matchesss;
$invID = 'V' . str_pad($prepocet, 6, '0', STR_PAD_LEFT) . '.xml';
}

}

//$nazov_suboru_new = 'V0000011.xml';

echo $invID;

$xml = simplexml_load_file($unzipDest . $nazov_suboru_new);
...
$subject = 'Import objednávok prebehol v poriadku ' . $nazov_suboru_new;
$body = implode("\n", $vyriabilne_simboly);
...
if( !wp_next_scheduled('ovrerenie_platby_slsp6' ) ) 
   wp_schedule_event( strtotime('14:05:00'), 'daily', 'ovrerenie_platby_slsp6' );
 
add_action( 'ovrerenie_platby_slsp6', 'manualne_napojenie_na_slsp_nove', 10, 3 );
